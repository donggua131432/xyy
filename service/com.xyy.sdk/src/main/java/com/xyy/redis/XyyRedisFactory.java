...
import com.xyy.exception.SystemException;
 ...
 * @版本号: V3.0
 */
public class XyyRedisFactory {

	private static String DEFAULT_REDIS_POOL = "default_redis_pool";

	private static Logger logger = LoggerFactory.getLogger(XyyRedisFactory.class);

	private static Map<String, XyyJedisPool> jedisCacheMap = new HashMap<>();

	private static Map<String, RedisService> redisServiceCache = new ConcurrentHashMap<>();

	private XyyRedisFactory() {

	}


	private static class MhsRedisFactoryHandler {
		private static XyyRedisFactory instance = new XyyRedisFactory();
	}


	public static XyyRedisFactory getInstance() {
	...
	public RedisService getRedisService() {
		...
		XyyJedisPool pool = getDefaultRedisPool();
	...
	public RedisService getRedisService(String name) {
		...
		XyyJedisPool pool = getRedisPool(name);
		RedisService redisService = RedisService.newRedisService(pool);
	 ...
	 * @param config
	 */
	public void init(XyyRedisBean config) {
		List<XyyRedisInfo> redisInfos = config.getRedisInfos();
		if (redisInfos != null && !redisInfos.isEmpty()) {
			...
			for (XyyRedisInfo redisInfo : redisInfos) {
	 ...
	 * @param timeOut
	 */
	private void initRedisPool(GenericObjectPoolConfig config, XyyRedisInfo redisInfo, int timeOut) {
		XyyJedisPool jedisPool = new XyyJedisPool(config, redisInfo.getHost(), redisInfo.getPort(), timeOut,
	 ...
	 * @param name
	 */
	private XyyJedisPool getRedisPool(String name) {
		for (XyyJedisPool redisPool : jedisCacheMap.values()) {
	 ...
	 * @return
	 */
	private XyyJedisPool getDefaultRedisPool() {
		for (XyyJedisPool redisPool : jedisCacheMap.values()) {
	...
	public static void destory() {
		for (XyyJedisPool jedisPool : jedisCacheMap.values()) {
